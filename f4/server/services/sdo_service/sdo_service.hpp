...
public:
  ...
  void send();
private:
  SdoAbortCode read_expedited(ODEntry const* od_entry,
                              ExpeditedSdo& tsdo,
                              ExpeditedSdo const& rsdo);
  SdoAbortCode write_expedited(ODEntry const* od_entry,
                               ExpeditedSdo& tsdo,
                               ExpeditedSdo const& rsdo);
  SdoAbortCode restore_default_parameter(ODObjectKey key);

  static ODObjectKey const restore_default_parameter_key;
};
