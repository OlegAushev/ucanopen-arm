...
ODObjectKey const SdoService::restore_default_parameter_key = {0x1011, 0x04};
...
SdoService::SdoService(impl::Server& server) : server_(server) {
  ...
  ucan::filter_config rsdo_filter = {
      .filter_number = 0,
      .filter_id_high = calculate_cob_id(Cob::rsdo, server_.node_id()) << 5,
      .filter_id_low = 0,
...
void SdoService::handle() {
  while (!rsdo_queue_.empty()) {
    ...
    ODEntry const* od_entry = server_.find_od_entry(key);
...
SdoAbortCode SdoService::read_expedited(ODEntry const* od_entry,
                                        ExpeditedSdo& tsdo,
                                        ExpeditedSdo const& rsdo) {
...
SdoAbortCode SdoService::write_expedited(ODEntry const* od_entry,
                                         ExpeditedSdo& tsdo,
                                         ExpeditedSdo const& rsdo) {
