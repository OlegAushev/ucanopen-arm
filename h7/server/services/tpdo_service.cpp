...
TpdoService::TpdoService(impl::Server& server) : _server(server) {
    for (size_t i = 0; i < _tpdo_msgs.size(); ++i) {
        ...
        _tpdo_msgs[i].header = {
            .Identifier = calculate_cob_id(to_cob_type(CobTpdo(i)),
                                           _server.node_id()),
...
void TpdoService::send() {
    ...
    for (size_t i = 0; i < _tpdo_msgs.size(); ++i) {
        if (!_tpdo_msgs[i].creator || _tpdo_msgs[i].period.count() <= 0) {
            continue;
        }
        if (now < _tpdo_msgs[i].timepoint + _tpdo_msgs[i].period) {
            continue;
        }

        canpayload_t payload = _tpdo_msgs[i].creator();
